...
class AppStore extends Component {
  state = {currentTabId: tabsList[0].tabId, searchInput: ''}

  getListBasedOnCategory = () => {
    const {currentTabId} = this.state
    console.log('current tab id :', currentTabId)
    const currentTabList = appsList.filter(
      eachApp => eachApp.category === currentTabId,
    )
    console.log('currentList', currentTabList)
  ...
  getCategoryListBasedOnSearch = currentTabCategoryList => {
    const {searchInput} = this.state
    const newList = currentTabCategoryList.filter(eachItem =>
      eachItem.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )
    return newList
  }

  onClickChangeTab = newTabId => {
    this.setState({currentTabId: newTabId})
  }

  render() {
    const {currentTabId} = this.state
    const currentTabCategoryList = this.getListBasedOnCategory()
    const filteredTabCategoryList = this.getCategoryListBasedOnSearch(
      currentTabCategoryList,
    )
    return (
      <div className="app_store_background_container">
        <h1 className="app_store_main_heading">App Store</h1>
        <div className="search_input_container">
          ...
          <img
            ...
            alt="search icon"
            className="search_image_element"
          />
        </div>
        <ul className="tab_list_container">
          {tabsList.map(eachTab => (
            <TabItem
              tabDetails={eachTab}
              key={eachTab.tabId}
              onClickChangeTab={this.onClickChangeTab}
              isTabActive={eachTab.tabId === currentTabId}
            />
          ))}
        </ul>
        <ul className="apps_list_container">
          {filteredTabCategoryList.map(eachAppItem => (
